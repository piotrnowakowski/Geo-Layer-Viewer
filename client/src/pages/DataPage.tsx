...
const LAYER_DATA_INFO: LayerDataInfo[] = [
  ...
  {
    id: "sites_elderly",
    methodology: "Population aged 65 and over by neighbourhood, derived from IBGE Censo 2022 data via the Sidra API. The service fetches age group tables (Table 9605 or 9514) for Porto Alegre (IBGE code 4314902) at the finest available geographic resolution. Where neighbourhood-level age data is matched (by name), the specific value is used; otherwise the city-wide elderly percentage from the Sidra municipality-level query is applied uniformly. Choropleth uses the existing IBGE neighbourhood polygon boundaries.",
    source: "IBGE — Brazilian Institute of Geography and Statistics, Censo 2022, via SIDRA API",
    sourceUrl: "https://servicodados.ibge.gov.br/api/v3/",
    date: "Censo 2022 (published 2023-2024)",
    resolution: "Neighbourhood (bairro) polygon boundaries; age data at finest available Sidra level",
    coverage: "Porto Alegre municipality — 90 neighbourhoods",
    notes: "IBGE Sidra typically provides demographic age structure at municipality or subdistrito level rather than bairro level. Where neighbourhood-level age data is unavailable, the city-wide estimate (currently ~15% elderly for Porto Alegre) is shown uniformly across all neighbourhoods — this will be improved as census microdata at finer resolution is released by IBGE. Elderly population is the primary heat-mortality risk group and a key criterion for climate vulnerability mapping. For higher-resolution spatial elderly population data, the WorldPop age-sex structure rasters (100m) can be used if raster processing (GDAL/rasterio) is available in the deployment environment.",
  },
  {
    id: "ref_modis_lst",
